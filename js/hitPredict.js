...
function getSongFeatures(songName) {
  ...
  function submitForm(event) {
    ...
    fetch('https://song-hit-predictor.onrender.com', {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      predValue = (data * 100).toFixed(2)

      const para = document.createElement("p")
      para.setAttribute("id", "result-para");
      const songNameInput =  document.getElementById("song-name-input");
      const txt = document.createTextNode(songNameInput.value + " is predicted to have a ")
      const highlightNum = document.createElement("span")
      highlightNum.textContent = predValue
      highlightNum.classList.add("highlight")
      const afterText = document.createTextNode("% chance of being appearing in the Billboard Hot 100.")

        para.appendChild(txt)
        para.appendChild(highlightNum)
        para.appendChild(afterText)

      document.getElementById("result-data").appendChild(para)
